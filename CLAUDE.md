...
> **Integration status (2026-07-05, GPU-verified).** Upstream's 2026-06-17 sync refactored the trainer into a unified, config-driven `FlexibleStrategy` (retiring the bespoke `TextToVideo`/`VideoToVideo` strategies) and replaced `validation_sampler.py` with `validation_runner.py`. Our audio-reference work has been **re-integrated onto that design** on branch `integrate-audio-onto-upstream` (off `main`) rather than merged as old-architecture code — so future upstream syncs stay low-friction. The bespoke `audio_reference.py` strategy and `validation_sampler.py` are RETIRED there; audio-reference training is now a `FlexibleStrategy` config (`configs/audio_reference_ic_lora.yaml`) plus a small `audio_positions_mode` extension shared by training AND eval. **Verified on the real 22B on a 4090**: config → int8-quanto → LoRA → `block_swap_blocks=24` all construct (startup peak 21.8 GB, post-attach 13.1 GB), and 3 synthetic forward+backward steps give finite loss with gradients flowing to the LoRA at 13.7 GB training peak. Still open: an actual training run + one `render_eval_sweep` render against a real trained checkpoint, and the observability loop layer (metrics/ref-gap/held-out-loss — unwired, non-blocking). `audio-guidance-iclora-vtv` is kept intact as the old-architecture reference. Sequencing + full detail: `internal/audio_iclora_upstream_integration.md`. Sections below marked "(pre-refactor)" describe the retired implementation; the mechanics still hold, only the wiring moved.
