...
from ltx_trainer.synthetic_av import generate_dataset, generate_dataset_paired_refs
...
def main() -> None:
    ...
    ap.add_argument("--paired-refs", action="store_true",
                    help="v2 mode: each row gets a SEPARATE reference clip with the same "
                         "visual identity but a different BPM (audio is load-bearing).")
    ...
    if args.paired_refs:
        captions = generate_dataset_paired_refs(
            args.out, args.n, bpm_range=(args.bpm_min, args.bpm_max),
            min_bpm_gap=args.min_bpm_gap,
            duration_s=args.duration, fps=fps, width=w, height=h, seed=args.seed,
        )
        print(f"Wrote {args.n} target clips + {args.n} reference clips + {captions} + manifest.jsonl under {args.out}")
    else:
